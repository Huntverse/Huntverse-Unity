...
- **UI (`LogInScreen`)**: 사용자의 입력(Input), 애니메이션, 이벤트에 따른 시각적 피드백만을 담당합니다.
- **Service (`LoginService`)**: 실제 데이터 처리, 패킷 생성, 서버 통신을 담당합니다.
...
- **`NotiConst`** 클래스가 이 에러 코드를 사용자에게 보여줄 '메시지'와 '색상(Color)'으로 변환하는 단일 소스(Source of Truth) 역할을 합니다.
...
### 4. 네트워크 매니저: `NetworkManager.cs` & `GameSession.cs`
*   **GameSession:** 게임의 라이프사이클을 관리하는 싱글톤입니다. `LoginService`를 초기화하고 보유하여, 게임 내 어디서든 로그인 서비스에 접근할 수 있게 합니다.
*   **NetworkManager:** `ConcurrentDictionary`를 사용해 로그인 서버, 게임 서버 등 다중 연결을 스레드 안전(Thread-Safe)하게 관리합니다.
